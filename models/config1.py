...
def p1m3(_g, step, sL, s, **kwargs):
    return {'param1': ['c'], 'param2': [10, 100]}
    # return {'param1': ['c'], 'param2': np.array([10, 100])}
def p2m3(_g, step, sL, s, **kwargs):
    return {'param1': ['d'], 'param2': [20, 200]}
    # return {'param1': ['c'], 'param2': np.array([10, 100])}


def proc_one_coef_A(_g, step, sL, s, **kwargs):
    return {'proc_one_coef_A': 0.7}

def proc_one_coef_B(_g, step, sL, s, **kwargs):
    return {'proc_one_coef_B': 1.3}
...
proc_one_coef_A = 0.7
proc_one_coef_B = 1.3
proc_one_coef_A_policy = policy('proc_one_coef_A', proc_one_coef_A)
proc_one_coef_B_policy = policy('proc_one_coef_B', proc_one_coef_B)

def es3(_g, step, sL, s, _input, **kwargs):
    y = 's3'
    x = s['s3'] * bound_norm_random(_input['seeds']['a'], _input['proc_one_coef_A'], _input['proc_one_coef_B'])
    return (y, x)

def es4(_g, step, sL, s, _input, **kwargs):
    y = 's4'
    x = s['s4'] * bound_norm_random(_input['seeds']['b'], _input['proc_one_coef_A'], _input['proc_one_coef_B'])
    return (y, x)
...
partial_state_update_blocks = [
    {
        "policies": {
            ...
            "b2": p2m1,
            "seeds": seeds_policy,
            "proc_one_coef_A": proc_one_coef_A_policy,
            "proc_one_coef_B": proc_one_coef_B_policy
        },
        "states": {
            "s1": s1m1,
            "s2": s2m1,
